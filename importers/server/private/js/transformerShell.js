...
function produceReportPage( outputPath, profileJSON){
	...
	var htmlFile = profileJSON.htmlFile;
	var dataFile = profileJSON.dataFile;
    ...
    "	</div>										\n"+ 
    "	<div id='DataTable'>Data Table space</div> 	\n"+              
	"<br/>\n"+
	"<!-- graph script -->\n"+
	"	 <script>\n"+
...
function produceDataFile (input, outputPath, profileJSON) {
	var fs = require('fs');
	var filename = profileJSON.dataFile;
	var target = outputPath + filename;
	var i, j;	
	// convert to JSON
	...
	if (output) {
		...
		console.log('writing data file ' + target);
					...
					for(var i=0; i < fileList.length; i++ ){
						...
						fs.readFile(profilePath + fileList[i], function (err, profileData) {
							if (err) {
								...
								produceDataFile(data, outputDataPath, profileJSON);
